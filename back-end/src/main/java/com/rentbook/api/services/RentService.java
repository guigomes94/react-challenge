...
public class RentService {
	...
	public List<RentDTO> calcPaymentValues(List<Rent> rents) {
		...
		for (Rent rent : rents) {
			...
			response.setId(rent.getIdRent());
			response.setClient(rent.getClient().getName());
	...
	public RentDTO devolutionBook(Long id, Rent obj) { 
		...
		if (oldObj.isPresent()) {
			...
			devolution.setId(updated.getIdRent());
			devolution.setClient(updated.getClient().getName());
